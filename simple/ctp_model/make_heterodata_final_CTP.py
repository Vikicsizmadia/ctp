...
class DataParserCTP:
    """Class to parse the CLUTRR dataset into HeteroData format"""

    def __init__(self,
                 train_path: str = join(dirname(dirname(dirname(dirname(abspath(__file__))))), 'data', 'clutrr-emnlp',
                                        'data_089907f8', '1.2,1.3_train.csv'),
                 test_paths: Optional[List[str]] = None,
                 device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):

        """On initialization this instance will contain the parsed datasets and other properties.

            Args:
                train_path (str): Path to the training dataset.
                test_paths (Optional[List[str]]): Path to the optional test datasets.
                device: The device onto which put the Tensors.
        """

        self.device = device

        with open(join(dirname(dirname(dirname(dirname(abspath(__file__))))), "ctp", "data", "clutrr-emnlp", "relations_store.yaml"),
                  'r') as f:
            rs = yaml.safe_load(f)
        ...
        (self._train_graph, self._train_nodes_ids, new_idx) = DataParserCTP.parse(train_path, 0,
                                                                                           self.device,
        ...
        for test_path in (test_paths if test_paths is not None else []):
            (self._test_graphs[test_path], self._test_nodes_ids[test_path], new_idx) = \
                DataParserCTP.parse(test_path, new_idx, self.device, self.edge_types_to_class) # edge_types)
        ...
        with open(path, newline='') as f:
            ...
            for row in reader:
                ...
                if len(_id) > 0:
                    ...
                    _story, _edge, _query = DataParserCTP._to_obj(story_edges), DataParserCTP._to_obj(edge_types), DataParserCTP._to_obj(query)
